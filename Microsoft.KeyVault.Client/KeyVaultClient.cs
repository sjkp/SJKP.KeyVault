...
using Microsoft.KeyVault.Client.Authentication;
...
namespace Microsoft.KeyVault.Client
{
    ...
    public class KeyVaultClient
    {
        ...
        public async Task<byte[]> BackupKeyAsync( string vault, string keyName )
        {
            ...
            using ( var httpResponse = await this.SendAsync<HttpRequestMessage>( "POST", CreateKeyUrl( keyIdentifier.BaseIdentifier, "backup" ) ).ConfigureAwait( false ) )
            {
        ...
        public async Task<KeyBundle> RestoreKeyAsync( string vault, byte[] keyBundleBackup )
        {
            ...
            using ( var httpResponse = await this.SendAsync<RestoreKeyRequestMessage>( "POST", CreateKeyVaultUrl( vault, "restore" ), request ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( httpResponse ).ConfigureAwait( false );
        ...
        public async Task<KeyBundle> CreateKeyAsync( string vault, string keyName, string keyType, int? keySize = null, string[] key_ops = null, KeyAttributes keyAttributes = null )
        {
            if ( string.IsNullOrEmpty( vault ) )
                throw new ArgumentNullException( "vault" );
            ...
            using ( var httpResponse = await this.SendAsync<CreateKeyRequestMessage>( "POST", CreateKeyUrl( keyIdentifier.BaseIdentifier, "create" ), request ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( httpResponse ).ConfigureAwait( false );
        ...
        public async Task<KeyBundle> DeleteKeyAsync( string vault, string keyName )
        {
            ...
            using ( var httpResponse = await this.SendAsync<HttpRequestMessage>( "DELETE", CreateKeyUrl( identifier.BaseIdentifier ) ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( httpResponse ).ConfigureAwait( false );
        ...
        public async Task<KeyBundle> GetKeyAsync( string keyIdentifier )
        {
            ...
            using ( var httpResponse = await this.SendAsync<HttpRequestMessage>( "GET", CreateKeyUrl( keyIdentifier, string.Empty ) ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( httpResponse ).ConfigureAwait( false );
        ...
        public async Task<IEnumerable<KeyItem>> GetKeysAsync( string vault )
        {
            ...
            using ( var response = await this.SendAsync<HttpRequestMessage>( "GET", CreateKeyVaultUrl( vault ) ).ConfigureAwait( false ) )
            {
        ...
        public async Task<KeyBundle> ImportKeyAsync( string vault, string keyName, KeyBundle keyBundle, bool? importToHardware = null )
        {
            ...
            var identifier = new KeyIdentifier( vault, keyName );
            var request    = new ImportKeyRequestMessage { Hsm = importToHardware, Key = keyBundle.Key, Attributes = keyBundle.Attributes };

            using ( var httpResponse = await this.SendAsync<ImportKeyRequestMessage>( "PUT", CreateKeyUrl( identifier.BaseIdentifier, "import" ), request ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( httpResponse ).ConfigureAwait( false );
        ...
        public async Task<KeyBundle> UpdateKeyAsync( string vault, string keyName, string[] keyOps = null, KeyAttributes attributes = null )
        {
            if ( string.IsNullOrEmpty( vault ) )
                throw new ArgumentNullException( "vault" );
        ...
        public async Task<KeyOperationResult> DecryptDataAsync( string keyIdentifier, string algorithm, byte[] cipherText )
        {
            ...
            var request  = new KeyOpRequestMessage { Alg = algorithm, Value = cipherText };

            using ( var response = await this.SendAsync<KeyOpRequestMessage>( HttpMethod.Post.Method, CreateKeyUrl( keyIdentifier, "decrypt" ), request ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );

                var decryptResponse = await DeserializeAsync<KeyOpResponseMessage>( response ).ConfigureAwait( false );

                return new KeyOperationResult( keyIdentifier, decryptResponse.Value );
            }
        }
        ...
        public async Task<KeyOperationResult> EncryptDataAsync( string vault, string keyName, string keyVersion, string algorithm, byte[] plainText )
        {
            ...
            return await EncryptDataAsync( keyIdentifier.Identifier, algorithm, plainText ).ConfigureAwait( false );
        }
        ...
        public async Task<KeyOperationResult> EncryptDataAsync( string keyIdentifier, string algorithm, byte[] plainText )
        {
            ...
            var request  = new KeyOpRequestMessage { Alg = algorithm, Value = plainText };

            using ( var response = await this.SendAsync<KeyOpRequestMessage>( HttpMethod.Post.Method, CreateKeyUrl( keyIdentifier, "encrypt" ), request ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );

                var encryptResponse = await DeserializeAsync<KeyOpResponseMessage>( response ).ConfigureAwait( false );

                return new KeyOperationResult( encryptResponse.Kid, encryptResponse.Value );
            }
        }
        ...
        public async Task<KeyOperationResult> SignAsync( string vault, string keyName, string keyVersion, string algorithm, byte[] digest )
        {
            ...
            return await SignAsync( keyIdentifier.Identifier, algorithm, digest ).ConfigureAwait( false );
        }
        ...
        public async Task<KeyOperationResult> SignAsync( string keyIdentifier, string algorithm, byte[] digest )
        {
            ...
            var request  = new KeyOpRequestMessage { Alg = algorithm, Value = digest };

            using ( var response = await this.SendAsync<KeyOpRequestMessage>( HttpMethod.Post.Method, CreateKeyUrl( keyIdentifier, "sign" ), request ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );

                var signResponse = await DeserializeAsync<KeyOpResponseMessage>( response ).ConfigureAwait( false );

                return new KeyOperationResult( keyIdentifier, signResponse.Value );
            }
        }
        ...
        public async Task<KeyOperationResult> UnwrapKeyAsync( string keyIdentifier, string algorithm, byte[] wrappedKey )
        {
            ...
            var request  = new KeyOpRequestMessage { Alg = algorithm, Value = wrappedKey };

            using ( var response = await this.SendAsync<KeyOpRequestMessage>( HttpMethod.Post.Method, CreateKeyUrl( keyIdentifier, "unwrapkey" ), request ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );

                var unwrapResponse = await DeserializeAsync<KeyOpResponseMessage>( response ).ConfigureAwait( false );

                return new KeyOperationResult( keyIdentifier, unwrapResponse.Value );
            }
        }
        ...
        public async Task<bool> VerifyAsync( string keyIdentifier, string algorithm, byte[] digest, byte[] signature )
        {
            ...
            var request  = new VerifyRequestMessage { Alg = algorithm, Digest = digest, Value = signature };

            using ( var response = await this.SendAsync<VerifyRequestMessage>( HttpMethod.Post.Method, CreateKeyUrl( keyIdentifier, "verify" ), request ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );

                var verifyResponse = await DeserializeAsync<VerifyResponseMessage>( response ).ConfigureAwait( false );

                return verifyResponse.Value;
            }
        }
        ...
        public async Task<KeyOperationResult> WrapKeyAsync( string vault, string keyName, string keyVersion, string algorithm, byte[] key )
        {
            ...
            return await WrapKeyAsync( keyIdentifier.Identifier, algorithm, key ).ConfigureAwait( false );
        }
        ...
        public async Task<KeyOperationResult> WrapKeyAsync( string keyIdentifier, string algorithm, byte[] key )
        {
            ...
            var request  = new KeyOpRequestMessage { Alg = algorithm, Value = key };

            using ( var response = await this.SendAsync<KeyOpRequestMessage>( HttpMethod.Post.Method, CreateKeyUrl( keyIdentifier, "wrapkey" ), request ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );

                var wrapResponse = await DeserializeAsync<KeyOpResponseMessage>( response ).ConfigureAwait( false );

                return new KeyOperationResult( wrapResponse.Kid, wrapResponse.Value );
            }
        }
        ...
        public async Task<IEnumerable<SecretItem>> GetSecretsAsync( string vault )
        {
            ...
            using ( var response = await this.SendAsync<HttpRequestMessage>( "GET", CreateSecretVaultUrl( vault ) ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );
        ...
        public async Task<Secret> GetSecretAsync( string vault, string secretName, string secretVersion = null )
        {
            ...
            using ( var response = await this.SendAsync<HttpRequestMessage>( "GET", CreateSecretUrl( identifier.Identifier ) ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );
        ...
        public async Task<Secret> GetSecretAsync( string secretIdentifier )
        {
            ...
            using ( var response = await this.SendAsync<HttpRequestMessage>( "GET", CreateSecretUrl( secretIdentifier ) ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );
        ...
        public async Task<Secret> DeleteSecretAsync( string vault, string secretName )
        {
            ...
            using ( var response   = await this.SendAsync<HttpRequestMessage>( "DELETE", CreateSecretUrl( identifier.BaseIdentifier ) ).ConfigureAwait( false ) )
            {
                await EnsureSuccessStatusCode( response ).ConfigureAwait( false );
